...
- 未知/边界：
  ...
  - 方法偏差披露：诊断 3 的查询串与 `queries.yaml` eval `zh2en-eval-01` 相同。这是 W2 接口诊断，不是 E1 运行：未做任何方法组比较、未做判定、未计入任何指标。E1 评估仍为未运行，正式运行前须冻结并另开批次；本条重合已在此披露，后续不得将本轮 `total_count`/前 3 当作 E1 证据引用。
