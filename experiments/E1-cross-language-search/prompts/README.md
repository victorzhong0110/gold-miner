...
- `c-rewrite.txt`：C 组「本方案（1a 脚本）」用。同一任务下，对一条用户原话执行一次，得到最多 2 条中文加最多 2 条英文查询。合并去重后取前 30。结果少于 10 条时是否追加 1 条 `in:readme` 变体，取决于协议第 1 节的前置探测结论。每任务最多 4 次 Search API 调用、最多 1 次模型调用。
