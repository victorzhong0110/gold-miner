...
对应协议：`experiments/E1-cross-language-search/protocol.md` 第 5 节（方法组）。

## 三个文件何时用

- `b-translate.txt`：B 组「简单译词」对照用。同一任务下，对一条用户原话执行一次，得到另一语言的一条查询。两条查询（原文一条＋译文一条）各取前 30，再合并去重。每任务最多 5 次 Search API 调用、最多 2 次模型调用。
- `c-rewrite.txt`：C 组「本方案（1a 脚本）」用。同一任务下，对一条用户原话执行一次，得到最多 2 条中文加最多 2 条英文查询。合并去重后取前 30。结果少于 10 条时是否追加 1 条 `in:readme` 变体，取决于协议第 4 节的前置探测结论。每任务最多 5 次 Search API 调用、最多 2 次模型调用。
